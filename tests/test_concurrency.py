...
def test_stock_race_condition(auth_client: TestClient):
    ...
    customer_id = auth_client.post("/customers/", json={"name": "Race Guy", "phone": f"888{uuid.uuid4().hex[:7]}"}).json()["id"]
    item_id = auth_client.post("/items/", json={
        "sku": f"RACE-{uuid.uuid4().hex[:4]}", "name": "Race Item", "selling_price": 100, "current_stock": 10
    }).json()["id"]
    
    # We will attempt 15 concurrent purchases of 1 stock each.
    def make_purchase():
        return auth_client.post("/sales/", json={
    ...
    assert success_count == 10, f"Expected 10 successes, got {success_count}. Failures: {fail_count}"
    assert fail_count == 5
...
def test_idempotency_concurrency(auth_client: TestClient):
    _setup_thread_safe_db(auth_client)
    # Setup
    resp = auth_client.post("/customers/", json={"name": "Idempotent Guy", "phone": f"999{uuid.uuid4().hex[:7]}"})
    assert resp.status_code == 200, resp.text
    customer_id = resp.json()["id"]
    
    resp_item = auth_client.post("/items/", json={
        "sku": f"IDEM-{uuid.uuid4().hex[:4]}", "name": "Idempotent Item", "selling_price": 50, "current_stock": 100
    })
