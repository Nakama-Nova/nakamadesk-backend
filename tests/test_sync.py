...
def test_sync_conflict_resolution(auth_client: TestClient):
    ...
    item_id = auth_client.post("/items/", json={
        "sku": f"CONF-{uuid.uuid4().hex[:4]}", "name": "Conflict Base", "selling_price": 50.0
    }).json()["id"]
    ...
    op_a = {
        "id": str(uuid.uuid4()),
        "entity": "item", "action": "update", "payload": {"id": item_id, "name": "Older Update"},
    ...
    op_b = {
        "id": str(uuid.uuid4()),
        "entity": "item", "action": "update", "payload": {"id": item_id, "name": "Newer Update"},
...
def test_sync_partial_failure(auth_client: TestClient):
    op_good = {
        "id": str(uuid.uuid4()),
        "entity": "item", "action": "create", "payload": {"sku": str(uuid.uuid4()), "name": "Good"},
        "updated_at": datetime.now(timezone.utc).isoformat()
    }
    op_bad = {
        "id": str(uuid.uuid4()),
        "entity": "unknown_entity", "action": "create", "payload": {},
        "updated_at": datetime.now(timezone.utc).isoformat()
    }
