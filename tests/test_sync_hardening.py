...
def test_sale_idempotency(auth_client: TestClient):
    ...
    item_resp = auth_client.post("/items/", json={
        "sku": "IDEM-SALE-001",
...
def test_updated_at_timestamp(auth_client: TestClient):
    ...
    # 1. Create an item
    resp = auth_client.post("/items/", json={
        "sku": "TS-ITEM-001",
        "name": "Timestamp Test",
        "current_stock": 10
    })
    ...
    # 2. Update item
    update_resp = auth_client.patch(f"/items/{item['id']}", json={"name": "Updated Name"})
...
def test_financial_decimal_precision(auth_client: TestClient):
    ...
    sale_resp = auth_client.post("/sales/", json={
    ...
    assert Decimal(str(data["tax_total"])) == Decimal("179.98") # 999.9 * 0.18 = 179.982
