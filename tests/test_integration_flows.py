...
def test_full_business_flow(auth_client: TestClient):
    ...
    rm_id = auth_client.post("/raw-materials/", json={
        "name": "E2E Wood", "unit": "CFT", "current_price": "100.0", "stock": "100.0"
    }).json()["id"]

    # 2. Create Item
    item_id = auth_client.post("/items/", json={
        "sku": "E2E-CHAIR", "name": "E2E Chair", "selling_price": 500.0, "current_stock": 10
    }).json()["id"]

    # 3. Create BOM (1 CFT wood @ $100 -> Cost = $100)
    auth_client.post("/bom/", json={
        "item_id": item_id, "material_id": rm_id, "required_qty": "1.0", "wastage_pct": "0.0"
    })

    # Validate cost is 100
    assert float(auth_client.get(f"/items/{item_id}").json()["production_cost"]) == 100.0

    # 4. Create Sale (2 qty -> Revenue 1000)
    auth_client.post("/sales/", json={
...
def test_workforce_and_sales_flow(auth_client: TestClient, db):
    ...
    worker = User(
        username=f"e2e_worker_{worker_id}", email=f"e2e_{worker_id}@ex.com",
    ...
    auth_client.post("/attendance/", json={
        "user_id": str(worker.id), "date": str(date.today()), "status": "present", "daily_wage": 500.0
    })

    # 3. Create an item and a sale
    item_id = auth_client.post("/items/", json={
        "sku": f"WF-SALE-{worker_id}", "name": "WF Item", "selling_price": 200.0, "current_stock": 5
    }).json()["id"]
