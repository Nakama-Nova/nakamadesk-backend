...
from app.schemas.sync import SyncOperation, SyncPushResponse, SyncOperationResult, SyncPullResponse
...
def process_push_sync(db: Session, operations: List[SyncOperation]) -> SyncPushResponse:
    ...
    for op in operations:
        ...
        if existing_log and existing_log.status == "success":
            success_results.append(SyncOperationResult(
        ...
        if not model_class:
            failed_results.append(SyncOperationResult(client_id=op.id, status="failed", error=f"Unknown entity: {op.entity}"))
            continue
        ...
        try:
            with db.begin_nested():
                if op.action == "create":
                    ...
                    existing_obj = db.query(model_class).filter(model_class.id == record_id).first()
                    if not existing_obj:
                    ...
                    db_obj = db.query(model_class).filter(model_class.id == record_id).first()
                    if not db_obj:
                    ...
                    if db_obj.updated_at and local_op_time < db_obj.updated_at:
                        ...
                        for key, value in op.payload.items():
                            ...
                            if key == "current_stock" and op.entity == "item":
                                db_obj.current_stock = getattr(db_obj, "current_stock", 0) + float(value)
                            else:
        ...
        # Logging to SyncLog
        if op_failed:
            failed_results.append(SyncOperationResult(client_id=op.id, status="failed", error=op_error))
            if not existing_log:
                db.add(SyncLog(client_id=op.id, entity=op.entity, record_id=record_id or uuid.uuid4(), action=op.action, payload=op.payload, status="failed", error_message=op_error))
            else:
                existing_log.status = "failed"
                existing_log.error_message = op_error
        else:
            success_results.append(SyncOperationResult(client_id=op.id, record_id=record_id, status="success"))
            if not existing_log:
                db.add(SyncLog(client_id=op.id, entity=op.entity, record_id=record_id, action=op.action, payload=op.payload, status="success"))
            else:
...
def pull_sync(db: Session, last_sync: datetime) -> SyncPullResponse:
    ...
    raw_materials = db.query(RawMaterial).filter(RawMaterial.updated_at > last_sync).all()
