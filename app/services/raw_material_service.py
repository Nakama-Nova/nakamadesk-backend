...
def update_raw_material(db: Session, material_id: UUID, material_update: RawMaterialUpdate) -> RawMaterial:
    ...
    price_changed = False
    if "current_price" in update_data and update_data["current_price"] != db_material.current_price:
        price_changed = True
    ...
    if price_changed:
        ...
        affected_items = db.query(BillOfMaterials.item_id).filter(BillOfMaterials.material_id == material_id).distinct().all()
